const ApiServices = {
  ...
  postAddGroceryItem(userID, authToken, items) {
    return fetch(`${process.env.REACT_APP_API_ENDPOINT}/users/cart/${userID}`, {
      method: "POST",
      headers: {
        "content-type": "application/json",
        Authorization: authToken,
  ...
  deleteGroceryItem(userID, authToken, itemID) {
    return fetch(
      `${process.env.REACT_APP_API_ENDPOINT}/users/cart/${userID}/${itemID}`,
      {
        method: "DELETE",
        headers: {
          Authorization: authToken,
